...
const Announcements = () => {
	...
	const fetchAnnouncements = async () => {
		try {
			...
			const res = await axios.get(`${BASE_URL}/api/announcement/getAll`, {
				headers: {
	...
	const handleEdit = (announcement) => {
		...
		setFormData({
			...
			description_uz: announcement.description_uz || '',
			description_ru: announcement.description_ru || '',
			description_en: announcement.description_en || '',
			photos: announcement.photos || []
		})
		setPhotoPreviews(announcement.photos?.map(photo => `${BASE_URL}${photo}`) || [])
		setSelectedPhotos([])
		setRemovedPhotos([])
		setShowForm(true)
	}
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			if (editingAnnouncement) {
				res = await axios.put(
					`${BASE_URL}/api/announcement/update/${editingAnnouncement._id}`,
					submitData,
					{
						headers: {
				...
				res = await axios.post(`${BASE_URL}/api/announcement/create`, submitData, {
					headers: {
						'Content-Type': 'multipart/form-data',
						Authorization: `Bearer ${token}`,
					},
			...
			if (res.data.success) {
				alert(`✅ E'lon ${editingAnnouncement ? "yangilandi" : "yaratildi"}!`)
				resetForm()
				fetchAnnouncements()
				setShowForm(false)
			}
	...
	const handleDelete = async (announcementId) => {
		...
		try {
			const token = localStorage.getItem("token")
			const res = await axios.delete(`${BASE_URL}/api/announcement/delete/${announcementId}`, {
				headers: {
					Authorization: `Bearer ${token}`,
				},
			})
	...
	const removeExistingPhoto = (announcementId, photoPath, index) => {
		if (!window.confirm("Bu rasmni o'chirmoqchimisiz?")) return

		if (editingAnnouncement && editingAnnouncement._id === announcementId) {
		...
		<div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-8 px-4">
			<div className="mx-auto">
				...
				<div className="text-center mb-8">
					<h1 className="text-3xl font-bold text-gray-800 mb-2">E'lonlar Boshqaruvi</h1>
				</div>
							...
							<div className="bg-blue-600 text-white p-6 rounded-t-xl">
								<div className="flex justify-between items-center">
									<h2 className="text-xl font-bold">
										{editingAnnouncement ? "E'loni Tahrirlash" : "Yangi E'lon Yaratish"}
									</h2>
									<button
								...
								<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
									<div>
										...
										<input
											...
											placeholder="E'lon sarlavhasi o'zbekcha"
								...
								<div className="space-y-4">
									<div>
										...
										<textarea
											...
											onChange={handleInputChange}
											rows="3"
											placeholder="E'lon tavsifi o'zbekcha"
													...
													<button
														...
														onClick={() => {
															if (preview.startsWith(BASE_URL)) {
																...
																if (photoIndex !== -1) {
																	removeExistingPhoto(editingAnnouncement._id, formData.photos[photoIndex], index)
																}
															} else {
								...
								<div className="flex space-x-3 pt-4">
									...
									>
										{loading ? (
											...
											editingAnnouncement ? "Yangilash" : "Yaratish"
										)}
									</button>
									<button
										type="button"
				...
				<div className="bg-white rounded-xl shadow-lg overflow-hidden">
					...
					<div className="text-black p-6">
						<h2 className="text-xl font-bold">Barcha E'lonlar</h2>
						<p className="text-black">
							{searchTerm ? `"${searchTerm}" qidiruvi: ${filteredAnnouncements.length} ta` : `Jami: ${filteredAnnouncements.length} ta`}
						</p>
					</div>
					...
					<div className="p-6">
						{filteredAnnouncements.length === 0 && !loading ? (
							<div className="text-center py-8">
								...
								<h3 className="text-gray-600 mb-2">E'lonlar topilmadi</h3>
								<p className="text-gray-500 text-sm mb-4">
									{searchTerm ? "Boshqa so'zlar bilan qidiring" : "Birinchi e'lonni yarating"}
								</p>
								{!searchTerm && (
									<button
										onClick={handleAddNew}
									...
									<div key={announcement._id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
										<div className="flex justify-between items-start">
											<div className="flex-1">
												<h3 className="font-semibold text-gray-800 mb-1">{announcement.title_uz}</h3>
												<p className="text-gray-600 text-sm mb-2 line-clamp-2">
													{announcement.description_uz}
												</p>
												<div className="flex items-center space-x-4 text-xs text-gray-500">
													<span>Yaratilgan: {new Date(announcement.createdAt).toLocaleDateString()}</span>
													<span>{announcement.photos?.length || 0} ta rasm</span>
												</div>
											</div>
