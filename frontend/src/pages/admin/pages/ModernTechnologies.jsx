...
const ModernTechnologies = () => {
	...
	const fetchTechnologies = async () => {
		try {
			...
			const res = await axios.get(`${BASE_URL}/api/technologies/getAll`, {
				headers: {
			...
			if (res.data?.technologies) {
				...
				setFilteredTechnologies(res.data.technologies)
			}
		} catch (err) {
			console.error("Texnologiyalarni olishda xatolik:", err.message)
		} finally {
			setLoading(false)
		}
	}
	...
	const handleEdit = (technology) => {
		...
		setFormData({
			...
			description_uz: technology.description_uz || '',
			description_ru: technology.description_ru || '',
			description_en: technology.description_en || '',
			photo: null
		})
		setPhotoPreview(technology.photo ? `${BASE_URL}${technology.photo}` : '')
		setSelectedPhoto(null)
		setShowForm(true)
	}
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			if (editingTechnology) {
				res = await axios.put(
					`${BASE_URL}/api/technologies/update/${editingTechnology._id}`,
					submitData,
					{
						headers: {
				...
				res = await axios.post(`${BASE_URL}/api/technologies/create`, submitData, {
					headers: {
						'Content-Type': 'multipart/form-data',
						Authorization: `Bearer ${token}`,
					},
			...
			if (res.data.success) {
				alert(`✅ Texnologiya ${editingTechnology ? "yangilandi" : "yaratildi"}!`)
				resetForm()
				fetchTechnologies()
				setShowForm(false)
			}
	...
	// 🗑️ Texnologiyani o'chirish
	const handleDelete = async (technologyId) => {
		if (!window.confirm("Haqiqatan ham bu texnologiyani o'chirmoqchimisiz?")) {
			return
		}

		try {
			const token = localStorage.getItem("token")
			const res = await axios.delete(`${BASE_URL}/api/technologies/delete/${technologyId}`, {
				headers: {
					Authorization: `Bearer ${token}`,
				},
			})
		...
		<div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-8 px-4">
			<div className="mx-auto">
				...
				<div className="text-center mb-8">
					<h1 className="text-3xl font-bold text-gray-800 mb-2">Zamonaviy Texnologiyalar Boshqaruvi</h1>
					<p className="text-gray-600">Suv xo'jaligi texnologiyalarini boshqaring va yangilarini qo'shing</p>
				</div>
							...
							<div className="bg-blue-600 text-white p-6 rounded-t-xl">
								<div className="flex justify-between items-center">
									<h2 className="text-xl font-bold">
										{editingTechnology ? "Texnologiyani Tahrirlash" : "Yangi Texnologiya Yaratish"}
									</h2>
									<button
								...
								<div className="flex space-x-3 pt-4">
									...
									>
										{loading ? (
											...
											editingTechnology ? "Yangilash" : "Yaratish"
										)}
									</button>
									<button
										type="button"
				...
				<div className="bg-white rounded-xl shadow-lg overflow-hidden">
					...
					<div className="text-black p-6">
						<h2 className="text-xl font-bold">Barcha Texnologiyalar</h2>
						<p className="text-black">
							{searchTerm ? `"${searchTerm}" qidiruvi: ${filteredTechnologies.length} ta` : `Jami: ${filteredTechnologies.length} ta`}
						</p>
					</div>
					...
					<div className="p-6">
						{filteredTechnologies.length === 0 && !loading ? (
							<div className="text-center py-8">
								...
								<h3 className="text-gray-600 mb-2">Texnologiyalar topilmadi</h3>
								<p className="text-gray-500 text-sm mb-4">
									{searchTerm ? "Boshqa so'zlar bilan qidiring" : "Birinchi texnologiyani yarating"}
								</p>
								{!searchTerm && (
									<button
										onClick={handleAddNew}
							...
							<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
								{filteredTechnologies.map((technology) => (
									<div key={technology._id} className="border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition-shadow bg-white">
										{/* Rasm */}
										<div className="h-48 overflow-hidden">
											<img
												src={`${BASE_URL}${technology.photo}`}
												alt={technology.title_uz}
										...
										{/* Kontent */}
										<div className="p-4">
											<h3 className="font-semibold text-gray-800 mb-2 line-clamp-2">{technology.title_uz}</h3>
											<p className="text-gray-600 text-sm mb-3 line-clamp-3">
												{technology.description_uz}
											</p>
											<div className="flex items-center justify-between">
												<div className="text-xs text-gray-500">
													ID: {technology._id.slice(-6)}
												</div>
