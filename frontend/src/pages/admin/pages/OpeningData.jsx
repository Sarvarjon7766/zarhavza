...
const OpeningData = () => {
	...
	const [filteredOpenDatas, setFilteredOpenDatas] = useState([])
	const [loading, setLoading] = useState(false)
	const [showForm, setShowForm] = useState(false)
	const [editingOpenData, setEditingOpenData] = useState(null)
	...
	// 🌐 Barcha open data ma'lumotlarini olish
	useEffect(() => {
		fetchOpenDatas()
	}, [])
	...
	useEffect(() => {
		if (searchTerm.trim() === '') {
			setFilteredOpenDatas(openDatas)
		} else {
			const filtered = openDatas.filter(item =>
			...
			setFilteredOpenDatas(filtered)
		}
	}, [searchTerm, openDatas])

	const fetchOpenDatas = async () => {
		try {
			setLoading(true)
			const token = localStorage.getItem("token")
			const res = await axios.get(`${BASE_URL}/api/opendata/getAll`, {
				headers: {
			...
			if (res.data && res.data.success) {
				setOpenDatas(res.data.opendatas || [])
				setFilteredOpenDatas(res.data.opendatas || [])
			} else {
				// Agar success false bo'lsa yoki ma'lumot bo'lmasa
				setOpenDatas([])
				setFilteredOpenDatas([])
			}
		} catch (err) {
			// 400 yoki boshqa xatolik bo'lsa, faqat console ga yozamiz, alert chiqarmaymiz
			console.log("Open data ma'lumotlarini olishda xatolik:", err.response?.data?.message || err.message)
			setOpenDatas([])
			setFilteredOpenDatas([])
		} finally {
			setLoading(false)
	...
	// ➕ Yangi open data qo'shish
	const handleAddNew = () => {
		setEditingOpenData(null)
		setFormData({
			title_uz: '',
			title_ru: '',
			title_en: '',
	...
	// ✏️ Open data ni tahrirlash
	const handleEdit = (openData) => {
		setEditingOpenData(openData)
		setFormData({
			title_uz: openData.title_uz || '',
			title_ru: openData.title_ru || '',
			title_en: openData.title_en || '',
			description_uz: openData.description_uz || '',
			description_ru: openData.description_ru || '',
			description_en: openData.description_en || ''
		})
		setShowForm(true)
	}
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			if (editingOpenData) {
				res = await axios.put(
					`${BASE_URL}/api/opendata/update/${editingOpenData._id}`,
					submitData,
					{
						headers: {
							Authorization: `Bearer ${token}`,
				...
				res = await axios.post(`${BASE_URL}/api/opendata/create`, submitData, {
					headers: {
						Authorization: `Bearer ${token}`,
					},
				})
			...
			if (res.data.success) {
				alert(`✅ Ma'lumot ${editingOpenData ? "yangilandi" : "yaratildi"}!`)
				resetForm()
				fetchOpenDatas()
				setShowForm(false)
			}
	...
	// 🗑️ Open data ni o'chirish
	const handleDelete = async (openDataId) => {
		...
		try {
			const token = localStorage.getItem("token")
			const res = await axios.delete(`${BASE_URL}/api/opendata/delete/${openDataId}`, {
				headers: {
					Authorization: `Bearer ${token}`,
				},
			})
			...
			<div className="mx-auto">
					...
					<div className="flex flex-col md:flex-row gap-4 items-center justify-between">
						...
						<div className="flex-1 w-full md:max-w-md">
							<div className="relative">
								<input
									type="text"
									placeholder="Ochiq ma'lumotlarni qidirish..."
									value={searchTerm}
							...
							<div className="bg-blue-600 text-white p-6 rounded-t-xl">
								<div className="flex justify-between items-center">
									<h2 className="text-xl font-bold">
										{editingOpenData ? "Ochiq Ma'lumotni Tahrirlash" : "Yangi Ochiq Ma'lumot Yaratish"}
									</h2>
									<button
									...
									<div className="space-y-4">
										<div>
											...
											<textarea
												...
												onChange={handleInputChange}
												rows="4"
												placeholder="Ochiq ma'lumotlar tavsifi..."
										...
										<div>
											...
											<textarea
												...
												onChange={handleInputChange}
												rows="4"
												placeholder="Описание открытых данных..."
												className="w-full px-3 text-black py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
												required
											/>
										</div>
										<div>
											...
											<textarea
												...
												onChange={handleInputChange}
												rows="4"
												placeholder="Open data description..."
												className="w-full px-3 py-2 text-black border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
												required
											/>
										</div>
								...
								<div className="flex space-x-3 pt-4">
									...
									>
										{loading ? (
											...
											editingOpenData ? "Yangilash" : "Yaratish"
										)}
									</button>
									<button
										type="button"
				...
				<div className="bg-white rounded-xl shadow-lg overflow-hidden">
					...
					<div className="text-black p-6">
						<h2 className="text-xl font-bold">Ochiq Ma'lumotlar</h2>
						<p className="text-black">
							{searchTerm ? `"${searchTerm}" qidiruvi: ${filteredOpenDatas.length} ta` : `Jami: ${filteredOpenDatas.length} ta`}
						</p>
					</div>

					{/* Open Data Items */}
					<div className="p-6">
						{filteredOpenDatas.length === 0 && !loading ? (
							<div className="text-center py-8">
								<svg className="w-16 h-16 text-gray-300 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
								</svg>
								<h3 className="text-gray-600 mb-2">Ochiq ma'lumotlar topilmadi</h3>
								<p className="text-gray-500 text-sm mb-4">
									{searchTerm ? "Boshqa so'zlar bilan qidiring" : "Birinchi ochiq ma'lumotni yarating"}
								</p>
								{!searchTerm && (
									<button
										onClick={handleAddNew}
									...
									>
										Yangi Ochiq Ma'lumot
									</button>
								)}
							</div>
						) : (
							<div className="space-y-6">
								{filteredOpenDatas.map((openData) => (
									<div key={openData._id} className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
										<div className="flex justify-between items-start mb-4">
											<div className="flex-1">
												{/* Title */}
												<div className="mb-3">
													<h3 className="font-semibold text-lg text-gray-800 mb-2">
														{openData.title_uz || "Sarlavha yo'q"}
													</h3>
													<div className="flex flex-wrap gap-2 text-xs">
														{openData.title_ru && (
															<span className="bg-blue-100 text-blue-800 px-2 py-1 rounded">RU: {openData.title_ru}</span>
														)}
														{openData.title_en && (
															<span className="bg-green-100 text-green-800 px-2 py-1 rounded">EN: {openData.title_en}</span>
														)}
													</div>
												</div>
												...
												<div className="flex items-center space-x-4 text-xs text-gray-500 mt-4 pt-3 border-t">
													<span>Yaratilgan: {new Date(openData.createdAt).toLocaleDateString()}</span>
													<span>Yangilangan: {new Date(openData.updatedAt).toLocaleDateString()}</span>
												</div>
											</div>
											...
											<div className="flex space-x-2 ml-4">
												...
												<button
													onClick={() => handleDelete(openData._id)}
