...
class GallaryService {
	async create(data) {
		try {
			const gallary = await gallaryModel.create(data)
			return gallary
	...
	async update(id, data) {
		try {
			...
			const updated = await gallaryModel.findByIdAndUpdate(id, data, { new: true })
	...
	async deleted(id) {
		try {
			...
			await gallaryModel.findByIdAndDelete(id)
	...
	async getAll(lang) {
		try {
			const gallarys = await gallaryModel.find().sort({ _id: -1 })
...
module.exports = new GallaryService()
