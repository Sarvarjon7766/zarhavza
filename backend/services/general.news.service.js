const generalnewsModel = require('../models/general.news.model')
const fs = require('fs')
...
class GeneralNewsService {
	async create(data) {
		try {
			const news = await generalnewsModel.create(data)
			if (news) {
	...
	async update(id, data) {
		try {
			const news = await generalnewsModel.findById(id)
			if (!news) {
			...
			// 🔹 yangilash
			const updated = await generalnewsModel.findByIdAndUpdate(id, data, { new: true })
	...
	async deleted(id) {
		try {
			...
			await generalnewsModel.findByIdAndDelete(id)
	...
	async getAll(key, lang) {
		try {
			const news = await generalnewsModel.find({ key }).sort({ _id: -1 })
...
module.exports = new GeneralNewsService()