const generalannouncementModel = require('../models/general.announcement.model')
const fs = require('fs')
const path = require('path')
class GeneralAnnouncementService {
	async create(data) {
		try {
			const announcement = await generalannouncementModel.create(data)
			if (announcement) {
	...
	async update(id, data) {
		try {
			const announcement = await generalannouncementModel.findById(id)
			if (!announcement) {
			...
			// 🔹 yangilash
			const updated = await generalannouncementModel.findByIdAndUpdate(id, data, { new: true })
	...
	async deleted(id) {
		try {
			...
			await generalannouncementModel.findByIdAndDelete(id)
	...
	async getAll(key, lang) {
		try {
			const announcements = await generalannouncementModel.find({ key }).sort({ _id: -1 })
...
module.exports = new GeneralAnnouncementService()