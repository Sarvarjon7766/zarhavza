...
class AnnouncementController {
	...
	async deleted(req, res) {
		try {
			const result = await AnnouncementService.deleted(req.params.id)
			if (result.success) {
				return res.status(201).json(result)
			} else {
				return res.status(400).json(result)
	...
	async getAlll(req, res) {
		try {
			const result = await AnnouncementService.getAlll()
			if (result.success) {
	...
	async getAll(req, res) {
		try {
			const result = await AnnouncementService.getAll(req.params.lang)
			if (result.success) {
				return res.status(200).json(result)
			} else {
				return res.status(400).json(result)
